...
def test_status_is_atomic_and_machine_readable(tmp_path,monkeypatch):
 monkeypatch.setattr(FinalIntegrationRuntime,'_trading',lambda self,cmd:{'status':'READY','profiles':[]})
 value=FinalIntegrationRuntime(tmp_path).status();payload=json.loads((tmp_path/'runtime/final_integration_status.json').read_text());assert value.schema_version=='afip-final-integration.v4';assert payload['architecture']['runtime_authorities']==['TRADING_RUNTIME','RESEARCH_RUNTIME']
