from afip.learning_review_certification import LearningReviewCertificationRuntime


def _row(i=1, **overrides):
    row = {
        "governance_validation_id": f"OGOV-{i:016X}",
...
def _evaluate(rows=None, **overrides):
    kwargs = {
        ...
        "review_record_id": "OREV-00000001",
        "review_timestamp": 5000,
        "review_outcome": "APPROVED_FOR_RESEARCH_CONTINUATION",
        "review_notes": "Manual research review completed and documented.",
    }
    kwargs.update(overrides)
    return LearningReviewCertificationRuntime().evaluate_many(rows or [_row(i) for i in range(1, 4)], **kwargs)


def test_ready_report_certifies_documented_manual_review():
    report = _evaluate()
...
def test_blocks_duplicate_governance_lineage():
    ...
    assert "duplicate_or_invalid_governance_validation_id" in _evaluate(rows).block_reasons
...
def test_blocks_invalid_review_chronology_and_future_leakage():
    ...
    assert "review_chronology_invalid" in report.block_reasons
...
def test_blocks_unaccepted_pack_8_governance():
    ...
    assert "pack_8_governance_not_accepted" in _evaluate(rows).block_reasons
...
def test_blocks_invalid_manual_review_documentation():
    ...
    assert "manual_review_record_invalid" in report.block_reasons
    assert "manual_review_outcome_not_approved" in report.block_reasons


def test_blocks_insufficient_coverage_and_confidence():
    report = _evaluate([_row(1, total_sample_count=1, mean_calibrated_confidence=20.0, minimum_calibrated_confidence=10.0)])
    assert "minimum_governance_report_count_not_met" in report.block_reasons
    assert "minimum_sample_count_not_met" in report.block_reasons
    assert "minimum_review_confidence_not_met" in report.block_reasons
