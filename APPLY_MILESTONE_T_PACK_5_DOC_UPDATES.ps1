...
$updates = @(
    @{ Source = "AFIP_PROJECT_DATABASE_MILESTONE_T_PACK_5_APPEND.md"; Target = "AFIP_PROJECT_DATABASE.md" },
    @{ Source = "HANDOFF_MILESTONE_T_PACK_5_APPEND.md"; Target = "HANDOFF.md" }
)
foreach ($update in $updates) {
    $sourcePath = Join-Path $root $update.Source
    $targetPath = Join-Path $root $update.Target
    $marker = "## Milestone T Pack 5"
