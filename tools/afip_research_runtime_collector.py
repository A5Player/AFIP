...
def activation_ledger_paths(profiles):
    return [profile.runtime_directory / "production_activation" / "activation_ledger.jsonl" for profile in profiles]
...
def main() -> int:
    ...
    activation_ledgers = activation_ledger_paths(profile for profile in profiles if profile.enabled and profile.research_enabled)
    report = ResearchRuntimeCollector(Path(args.output)).ingest_ledgers(ledgers, activation_ledger_paths=activation_ledgers)
