...
function Add-Once {
    ...
    if ($existing.IndexOf($Marker, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
        Write-Host "Already present: $Target"; return
    }
    Add-Content -LiteralPath $Target -Value "`r`n$appendContent" -Encoding UTF8
    Write-Host "Appended: $Target"
}
Add-Once -Target (Join-Path $root "AFIP_PROJECT_DATABASE.md") -AppendFile (Join-Path $root "AFIP_PROJECT_DATABASE_PACK_6_3_APPEND.md") -Marker "Milestone S Pack 6.3"
Add-Once -Target (Join-Path $root "HANDOFF.md") -AppendFile (Join-Path $root "HANDOFF_PACK_6_3_APPEND.md") -Marker "Milestone S Pack 6.3 Handoff"
Write-Host "Pack 6.3 documentation updates complete."
