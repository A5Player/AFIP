...
def render_research_operations(root: str | Path = ".") -> str:
    ...
    cards = [
        ("ðŸ“¥", "MT5 Closed Bars", f"{mt5_bars:,}", "Collected in current/latest cycle"),
        ("ðŸ—„ï¸", "Lake Appended", f"{appended:,}", f"Duplicates protected: {duplicates:,}"),
        ("â¯ï¸", "Replay Processed", f"{replay_processed:,}", progress_label),
        ("ðŸ§ª", "Research Candidates", f"{candidates:,}", "Classified candidates"),
        ("ðŸ†", "Ranking Records", f"{rankings:,}", "Generated ranking evidence"),
        ("âš ï¸", "Data Gaps", f"{gaps:,}", f"Missing bars: {missing:,}"),
        ("âš¡", "Replay Speed", f"{speed} bars/s", f"ETA: {eta if eta is not None else 'CALCULATING'} s"),
        ("ðŸ•¯ï¸", "Current Candle", candle, "Last processed replay candle"),
    ]
    card_html = "".join(
    ...
    for tf in TIMEFRAMES:
        ...
        active_label = ' <span class="active-dot">ACTIVE</span>' if tf == current_tf else ""
        rows.append(
            f'<tr><td><b>{tf}</b>{active_label}</td>'
...
</style></head><body><div class="page"><header><div class="toolbar"><a href="afip_profiles_dashboard.html">ðŸ“Š Operations</a><a href="afip_intelligence_engine_dashboard.html">ðŸ§  Intelligence</a><a href="afip_research_data_dashboard.html">ðŸ”¬ Research & Ranking</a><button onclick="location.reload()">â†» Refresh</button></div><h1>ðŸ“¡ AFIP Dashboard 4 Â· Data Loading & Research Operations</h1><p class="sub">Live read-only evidence for MT5 history, Data Lake, replay, research and ranking.</p><div class="status-row"><div class="status-box"><small>Runtime Status</small><strong><span class="badge {_status_class(overall)}">{escape(overall)}</span></strong></div><div class="status-box"><small>Current Stage</small><strong>{escape(stage)}</strong></div><div class="status-box"><small>Current Timeframe</small><strong>{escape(current_tf)}</strong></div><div class="status-box"><small>Last Heartbeat</small><strong>{escape(updated)}</strong></div></div><div class="reason"><b>Current activity:</b> {escape(reason)}</div></header><section class="metrics">{card_html}</section><section class="panel"><div class="panel-head"><h2>â³ Overall Replay Progress</h2><strong>{pct:.1f}%</strong></div><div class="overall-progress"><span style="width:{pct:.1f}%"></span></div><p class="foot">{escape(progress_label)} Â· Dashboard is read-only and has no execution authority.</p></section><section class="panel"><div class="panel-head"><h2>ðŸ•’ M1â€“D1 Timeframe Pipeline</h2><span class="badge {_status_class(overall)}">AUTO REFRESH 5S</span></div><div style="overflow:auto"><table><thead><tr><th>Timeframe</th><th>Available</th><th>Processed</th><th>Covered</th><th>Gaps / Missing</th><th>Progress</th><th>Status</th></tr></thead><tbody>{''.join(rows)}</tbody></table></div></section><section class="panel"><h2>ðŸ§¾ Recent Runtime Activity</h2><div style="overflow:auto"><table><thead><tr><th>Runtime file</th><th>Size</th><th>Last written</th></tr></thead><tbody>{recent_html}</tbody></table></div></section><p class="foot">Generated {escape(generated)} Â· Source: {escape(str(status.get('status_source','runtime/research/automatic_research_status.json')))} Â· Live execution authority: DISABLED Â· execution_authority=false Â· order_send_called=false</p></div></body></html>'''
