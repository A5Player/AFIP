...
def _profile_rows(profile: Mapping[str, Any]) -> list[tuple[str,str,str]]:
    ...
    return [
        ...
        ("0.01","Capital / 0.01",_financial(profile,"capital_per_unit","capital_per_001","capital_per_lot_unit")),
        ("#️⃣","Maximum units",_value(_first(profile,"maximum_units","max_units","profile_max_units",default="DATA_UNAVAILABLE"))),
        ("🪜","Current tier",_value(_first(profile,"current_tier_minimum_balance","current_tier",default="DATA_UNAVAILABLE"))),
        ("📦","Tier lots",_tier_lots(profile)),
        ("➡️","Next tier / Remaining",f"{_value(_first(profile,'next_tier_balance',default='MAX'))} / {_value(_first(profile,'remaining_to_next_tier',default='DATA_UNAVAILABLE'))}"),
...
class ThreeDashboardRuntime:
    def _profiles(self, record: Mapping[str,Any])->list[Mapping[str,Any]]:
        ...
        return list(FourProfileSupervisor(record.get("four_profile_config_path","config/four_profile_demo.json")).status().profiles)[:4]
    ...
    def render_profiles_html(self, record: Mapping[str,Any])->str:
        ...
        return f'''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="5"><title>AFIP P1-P4</title><style>{_base_style()}</style></head><body><div class="page"><header><div class="toolbar"><a href="{DASHBOARD_2_FILENAME}">🧠 Intelligence & Engines</a><a href="{DASHBOARD_3_FILENAME}">🔬 Research & Data</a><span class="status-pill">🔄 5 SEC</span></div><h1>📊 AFIP Dashboard 1 · P1–P4</h1><p><b>Runtime {running}/4 · MT5 {connected}/4</b> · Financial values are shown only when provided by the profile/MT5 runtime. Missing values remain DATA_UNAVAILABLE.</p><p class="small">{generated}</p>{e}</header><section class="section"><table class="profile-table"><thead><tr><th class="icon">◉</th><th class="metric">Metric</th>{heads}</tr></thead><tbody>{''.join(body)}</tbody></table></section><div hidden>P1 — Profile 1 | P2 — Profile 2 | P3 — Profile 3 | P4 — Profile 4 | AFIP Dashboard 1 — P1–P4 Operational Detail | AFIP Dashboard — Milestone H Pack 9 | AFIP Dashboard — Milestone H Pack 10</div></div></body></html>'''
