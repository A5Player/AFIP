...
def _operational_profile_cards_html(report: Any) -> str:
    ...
    for profile in report.profiles:
        ...
        cards.append(f'''<article class="profile-card {visual}"><div class="profile-title"><h2>{escape(str(profile.get("profile_id")))} — {escape(str(profile.get("profile_name")))}</h2><span class="badge {visual}">{escape(headline)}</span></div><div class="profile-primary">{escape(runtime_state)} · MT5 {escape(mt5_status)}</div><div class="profile-reason"><strong>Waiting / Status reason:</strong><br/>{escape(str(reason))}</div><table class="profile-meta"><tr><td>Account</td><td>{escape(str(profile.get("account")))}</td></tr><tr><td>Server</td><td>{escape(str(profile.get("server")))}</td></tr><tr><td>Runtime</td><td>{escape(str(profile.get("runtime_kind", "STOPPED")))} · PID {escape(str(profile.get("pid") or "-"))}</td></tr><tr><td>Demo verified / armed</td><td>{escape(str(profile.get("demo_verified", False)))} / {escape(str(profile.get("demo_armed", False)))}</td></tr><tr><td>Decision</td><td>{escape(str(profile.get("decision_action", "WAIT")))} · confidence {escape(str(profile.get("decision_confidence", 0.0)))}%</td></tr><tr><td>Order</td><td>{escape(str(profile.get("demo_order_status", "ORDER_NOT_SENT")))} · units {escape(str(profile.get("demo_sent_units", 0)))}</td></tr><tr><td>Tickets</td><td>{escape(ticket_text)}</td></tr><tr><td>Capital policy</td><td>{escape(str(profile.get("allocation_mode", "UNKNOWN")))}</td></tr><tr><td>Balance / current tier</td><td>{escape(str(profile.get("account_balance", 0.0)))} / {escape(str(profile.get("current_tier_minimum_balance") if profile.get("current_tier_minimum_balance") is not None else "N/A"))}</td></tr><tr><td>Tier allocation</td><td>{escape(" + ".join(f"{float(x):.2f}" for x in (profile.get("target_tier_lots") or [])) or "N/A")}</td></tr><tr><td>Next tier / remaining</td><td>{escape(str(profile.get("next_tier_balance") if profile.get("next_tier_balance") is not None else "MAX"))} / {escape(str(profile.get("remaining_to_next_tier", 0.0)))}</td></tr><tr><td>Withdrawal reference</td><td>{escape(str(profile.get("withdrawal_reference_balance") if profile.get("withdrawal_reference_balance") is not None else "N/A"))}</td></tr><tr><td>Latency</td><td>{escape(str(profile.get("latency_ms") if profile.get("latency_ms") is not None else "waiting"))} ms · reconnect {escape(str(profile.get("reconnect_attempts", 0)))}</td></tr><tr><td>Last update</td><td>{escape(str(profile.get("checked_at_utc", "NOT_RECORDED")))}</td></tr><tr><td>Freshness</td><td class="{freshness_class}">{freshness} · age {escape(age_text)}</td></tr></table></article>''')
