"""Milestone O Pack 9: deterministic Learning Review Certification.

Certifies that accepted Pack 8 governance reports received documented manual
...
class LearningReviewCertificationReport:
    status: str
    reason: str
    milestone: str
    pack: str
...
class LearningReviewCertificationRuntime:
    """Certify documented manual review of accepted Pack 8 governance reports."""
    ...
    def evaluate_many(
        ...
        *,
    ...
    ) -> LearningReviewCertificationReport:
        ...
        unique = bool(ids) and all(identifier.startswith("OGOV-") for identifier in ids) and len(ids) == len(set(ids))
        ...
        review_record_valid = record_id.startswith("OREV-") and len(record_id) >= 10 and notes_present and review_ts > 0
        ...
        metric_keys = ("total_sample_count", "mean_calibrated_confidence", "minimum_calibrated_confidence")
        ...
        mean_confidence = self._weighted_mean(rows, "mean_calibrated_confidence", "total_sample_count")
        ...
        checks = (
            ...
            (not unique, "duplicate_or_invalid_governance_validation_id"),
            (not accepted_pack_8, "pack_8_governance_not_accepted"),
            (not chronology, "review_chronology_invalid"),
            (not reviewer_valid, "manual_reviewer_identity_invalid"),
            (not review_record_valid, "manual_review_record_invalid"),
            (not outcome_valid, "manual_review_outcome_not_approved"),
            (not quality, "data_quality_not_certified"),
            (not future_safe, "future_leakage_detected"),
            (not finite, "non_finite_review_metric"),
            (not report_count_met, "minimum_governance_report_count_not_met"),
            (not sample_count_met, "minimum_sample_count_not_met"),
            (not confidence_met, "minimum_review_confidence_not_met"),
        ...
        certification_id = "OCERT-" + sha256(json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()).hexdigest()[:16].upper()

        if certified:
            reason = "LEARNING_REVIEW_CERTIFIED_FOR_RESEARCH_CONTINUATION"
            en = "Accepted Pack 8 governance reports received documented manual review and are certified only for continued Milestone O research validation."
            th = "รายงาน Governance จาก Pack 8 ได้รับการทบทวนด้วยมนุษย์พร้อมเอกสาร และได้รับการรับรองเฉพาะสำหรับการตรวจงานวิจัยต่อใน Milestone O"
            next_en = "Proceed to Milestone O Pack 10 certification; keep parameter updates, production promotion, and execution disabled."
            next_th = "ดำเนินการต่อไปยัง Milestone O Pack 10 โดยคงการปรับ Parameter การเลื่อนสู่ Production และ Execution ไว้ที่ปิด"
        else:
            reason = "LEARNING_REVIEW_CERTIFICATION_BLOCKED"
        ...
        return LearningReviewCertificationReport(
            status="READY" if certified else "BLOCKED", reason=reason, milestone="O", pack="9",
            review_certification_id=certification_id, governance_validation_ids=ids,
            governance_report_count=len(rows), total_sample_count=total_samples,
            ...
            future_safe=future_safe, finite_metrics=finite, locked_policy_valid=policies_valid,
