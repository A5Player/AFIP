"""Completion evidence models for AFIP Production Milestone C."""
...
def _status(value: object) -> str:
    ...
    if normalized in {"WAIT", "PENDING", "REVIEW"}:
        return "WAIT"
    if normalized in {"FAIL", "FAILED", "BLOCK"}:
        return "BLOCK"
...
class MilestoneCapabilityEvidence:
    pack_id: int
    capability_name: str
    status: str
    quality_status: str
    ...
    def completion_key(self) -> str:
        return f"PACK_{self.pack_id}|{self.sequence_order}|{self.capability_name.upper()}"
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "MilestoneCapabilityEvidence":
        return cls(
