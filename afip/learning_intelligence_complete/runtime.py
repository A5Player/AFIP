"""Milestone O Pack 10: deterministic Learning Intelligence completion certification.

Closes Milestone O by validating capability lineage for Packs 1-9 and accepted
...
class LearningIntelligenceCompleteReport:
    status: str
    reason: str
    milestone: str
    pack: str
    ...
    dataset_roles_separated: bool
    chronology_valid: bool
    feature_freeze_valid: bool
    learning_intelligence_complete: bool
...
class LearningIntelligenceCompleteRuntime:
    """Certify Milestone O completion without granting adaptive authority."""

    REQUIRED_PACKS = tuple(range(1, 10))

    def evaluate_one(self, payload: Mapping[str, Any]) -> LearningIntelligenceCompleteReport:
        data = dict(payload)
        ...
        unique_lineage = bool(pack_ids) and len(pack_ids) == len(set(pack_ids)) and all(
            identifier.startswith("OLEARN-") for identifier in pack_ids
        )
        ...
        roles_separated = bool(data.get("dataset_roles_separated", False)) and all(
            bool(row.get("dataset_roles_separated", False)) for row in capability_rows
        )
        ...
        checks = (
            (not packs_complete, "milestone_o_pack_1_to_9_incomplete"),
            (not unique_lineage, "duplicate_or_invalid_learning_capability_lineage"),
            (not review_certified, "pack_9_review_certification_not_accepted"),
            ...
            (not roles_separated, "dataset_role_separation_not_certified"),
            (not chronology, "learning_completion_chronology_invalid"),
        ...
        completion_id = "OCOMP-" + sha256(
            json.dumps(identity, sort_keys=True, separators=(",", ":")).encode()
        ).hexdigest()[:16].upper()

        if complete:
            reason = "LEARNING_INTELLIGENCE_COMPLETE"
            en = "Milestone O Packs 1-9 and documented manual review were certified as complete for research-only progression."
            th = "Milestone O Pack 1-9 และการทบทวนด้วยมนุษย์ที่มีเอกสารได้รับการรับรองว่าครบถ้วนสำหรับการดำเนินงานวิจัยต่อเท่านั้น"
            next_en = "Proceed to Milestone P Pack 1 — Market Behaviour Intelligence Foundation; production certification remains pending Milestone R."
            next_th = "ดำเนินการต่อไปยัง Milestone P Pack 1 — Market Behaviour Intelligence Foundation โดย Production Certification ยังคงรอ Milestone R"
        else:
            reason = "LEARNING_INTELLIGENCE_COMPLETION_BLOCKED"
            en = "Milestone O completion was blocked because capability lineage, review certification, chronology, data integrity, future safety, deterministic controls, dataset separation, or frozen policy failed."
            th = "การปิด Milestone O ถูกระงับเนื่องจาก Capability lineage, Review certification, ลำดับเวลา, Data Integrity, Future Safety, Deterministic Control, การแยก Dataset หรือนโยบายล็อกไม่ผ่าน"
        ...
        return LearningIntelligenceCompleteReport(
            ...
            dataset_roles_separated=roles_separated,
            chronology_valid=chronology,
            feature_freeze_valid=policy_valid,
            learning_intelligence_complete=complete,
